...
st.markdown("# Calculadora de pressão assistencial em decorrência da COVID-19")
e = 1e-50
...
hospital_beds_total = st.sidebar.number_input(
    "Total", min_value=0, value=int(region.hospital_total_capacity),
)
...
icu_beds_total = st.sidebar.number_input(
    "Total", min_value=0, value=int(region.icu_total_capacity), key="icu_total",
)
...
model = SEICHAR(region=region, **kwargs)
model.run(period)
